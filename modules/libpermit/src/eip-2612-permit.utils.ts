...
export class Eip2612PermitUtils {
  ...
  async recoverPermitOwnerFromCallData(params: PermitRecoveryParams): Promise<string> {
    const { owner } = this.connector.decodeABIParameters(EIP_2612_PERMIT_INPUTS, params.callData);
  ...
  syncRecoverPermitOwnerFromCallData(params: SyncPermitRecoveryParams): string {
    ...
    const { owner, spender, value, deadline, v, r, s } = this.connector.decodeABIParameters(
      EIP_2612_PERMIT_INPUTS,
      callData,
    );
  ...
  syncRecoverDaiLikePermitOwnerFromCallData(params: SyncPermitRecoveryParams): string {
    ...
    const { holder, spender, value, expiry, allowed, v, r, s } = this.connector.decodeABIParameters(
      DAI_EIP_2612_PERMIT_INPUTS,
      callData,
    );
  ...
  async getDomainTypeHash(tokenAddress: string): Promise<string | null> {
    try {
      return await this.connector.ethCall(
        tokenAddress,
        this.connector.contractEncodeABI(DOMAIN_TYPEHASH_ABI, tokenAddress, 'DOMAIN_TYPEHASH', []),
      );
    } catch (e) {
  ...
  async getPermitTypeHash(tokenAddress: string): Promise<string | null> {
    try {
      return await this.connector.ethCall(
        tokenAddress,
        this.connector.contractEncodeABI(PERMIT_TYPEHASH_ABI, tokenAddress, 'PERMIT_TYPEHASH', []),
      );
    } catch (e) {
      return Promise.resolve(null);
    }
  ...
  async getDomainSeparator(tokenAddress: string): Promise<string> {
    return await this.connector.ethCall(
      tokenAddress,
      this.connector.contractEncodeABI(DOMAIN_SEPARATOR_ABI, tokenAddress, 'DOMAIN_SEPARATOR', []),
    );
  }
  ...
  private getTokenNonceByMethod(
    ...
    const callData = this.connector.contractEncodeABI(ERC_20_NONCES_ABI, tokenAddress, methodName, [
      walletAddress,
    ]);
